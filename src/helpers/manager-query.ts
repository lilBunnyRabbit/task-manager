...
interface ManagerInstance<TParsed> {
  tasks: Task<TaskSpec, TParsed>[];
  queue: Task<TaskSpec, TParsed>[];
}

export class ManagerQuery<TParsed> {
  constructor(readonly manager: ManagerInstance<TParsed>) {}
  ...
  public findTask<TSpec extends TaskSpec>(taskBuilder: TaskBuilder<TSpec, TParsed>) {
    for (let i = 0; i < this.manager.tasks.length; i++) {
      const task: unknown = this.manager.tasks[i];
  ...
  public findLastTask<TSpec extends TaskSpec>(taskBuilder: TaskBuilder<TSpec, TParsed>) {
    for (let i = this.manager.tasks.length - 1; i >= 0; i--) {
      const task: unknown = this.manager.tasks[i];
      if (isTask(task, taskBuilder)) {
        return task;
      }
  ...
  public findTasks<TSpec extends TaskSpec>(taskBuilder: TaskBuilder<TSpec, TParsed>) {
    return this.manager.tasks.filter((task) => isTask<TSpec, TParsed>(task, taskBuilder)) as unknown as Task<
      TSpec,
      TParsed
    >[];
  }
